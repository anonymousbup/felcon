...
- Removed generic suggestion for outdated/old felcon release, instead show to screen output
...
- Russian translation of felcon extended
...
- Corrected issue when felcon is not executed directly from felcon directory
...
felcon now runs non-interactive by default, to be more in line with the Unix
...
- Security: test if setuid bit is set on felcon binary
...
- New command: felcon generate systemd-units
- Sending USR1 signal to felcon process will show active status
...
- Use 'pre-release/release' (was: 'dev/final') with 'felcon show release'
- Use only locations from PATH environment variable, unless it is not defined
- Show tip to use 'felcon generate hostids' when host IDs are missing
...
- Extended output of 'felcon update info'
...
- Command 'felcon show options' provides up-to-date list
...
- Changed command output when using 'felcon audit system remote'
...
- New URL structure on CISOfy website implemented for felcon controls
...
- Changed 'felcon audit system remote' routine
- Support for macOS High Sierra
- French translation updated

felcon Enterprise:
-----------------
- Allow 'tags' and 'system-customer-name' to be specified via felcon client
...
* New command: felcon show hostids (show host ID)
* New command: felcon show environment (hardware, VM, or container type)
* New command: felcon show os (show operating system details)
...
* Instead of opening the log file, you can now use 'felcon show details' followed
...
Commands: felcon show categories, felcon show groups
...
the initialization phases of felcon. It will perform a strict code check for the
tests, to detect any uninitialized variables, improving code quality.


Helpers
-------
With 'felcon update check' you can now check for updates. This is the preferred
new method.

The command 'felcon show changelog' allows reviewing the changes. Optionally a
...
* Extended tests database with category (felcon show tests)
* By default felcon will now run in 'quick mode' and not break after each
...
changes have been made to core functions of felcon. These changes are the next of
...
default.prf, copy your changes to custom.prf. Use 'felcon show profiles' to show
...
'felcon show settings' to see if they are applied.

Check your cron jobs:
When using --quiet, the output will be really quiet now. Use --show-warnings-only
if you still want to see the warnings. felcon will now exit with error 0, even
...
Databases
---------
felcon will check also for DB2 instances and report the status.
...
Examples:
felcon audit system --profile developer.prf
felcon audit system --developer
...
To simplify the usage of felcon, a new helper utility has been added: show.
This helper will show help, or values (e.g. version, plugin directories, etc).
Some examples include: felcon show options, felcon show commands, felcon show
version, etc. See felcon show for all available details.
...
Language Support
----------------
felcon now supports language translations, with the language profile option.
...
Profiles
--------
felcon now support multiple profiles. By using a file 'custom.prf', it allows to
...
Remote scanning
---------------
Although felcon is a aimed on running on local hosts, there is still an ongoing
demand for running remote scans. With 'felcon audit system remote' tips are now
...
* --config              - Removed: use 'felcon show profiles' instead


Functions
---------
* AddSetting            - New function to store settings (felcon show settings)
...
* Authentication
----------------
Depending on the operating system, felcon now tries to determine if failed logins
...
* Logging
---------
Extended the test syslog-ng logging to remote systems. The log felcon itself
...
- felcon now uses different exit codes, depending on errors or finding warnings.
  This helps with automation and any custom scripting you want to apply.
- Preparations to allow compressing the felcon report file and enhance uploads.
- Added --config option to show what settings file or profile is used.
- Tool tips are displayed, to make felcon even easier to use.
...
* Plugins
---------
[PAM]       New plugin available in all versions of felcon
...
* Automatic updates
-------------------
Initial work on an automatic updater has been implemented. This way felcon
...
felcon by providing a single goal. The first helper available is to audit
Docker build files.

felcon Improved OS support
-------------------------
...
- Running felcon without root permissions will start non-privileged scan
...
felcon 1.6.2 
 ...
 - Support for drill binary to check for felcon update
 ...
 - Bash completion script for felcon
...
felcon 1.6.1 
 ...
 - Changes to parameters and functions, to allow penetration tests with felcon
...
felcon 1.5.5 
 ...
 our blog post: http://linux-audit.com/felcon-security-notice-154-and-older/
...
felcon 1.4.3 
 ...
 New:
 - Support for ClearOS
 - Data upload for felcon Enterprise users (--upload)
...
felcon 1.2.2 
 ...
 - felcon displays a warning now, if current version is really outdated
...
felcon 1.1.3 
 ...
 - Added /etc/felcon/plugins to searchable plugin directory targets
 ...
 ** Special release notes [package/ports]: **
 - Added several default paths to check for usable INCLUDE directory. This
   should make packaging felcon easier for downstream package providers.
 - When no profile is set, felcon will check first /etc/felcon/default.prf,
...
felcon 1.0.7 
 ...
 - Logging extended
 - Updated felcon.spec file (contrib)
 ...
 - Deleted felcon.spec file, since it was not working and will be rewritten later
...
felcon 1.0.5 
 ...
 - Support added to run felcon as cronjob (--cronjob)
...
felcon 1.0.2 
 ...
 - Updated list of parameters within felcon help
...
felcon 1.0.0 
 ...
 - Added felcon.spec file for RPM creation
