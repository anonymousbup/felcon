...
#########################################################################
#
# Builds felcon distribution
#
# Usage: this script creates felcon builds
#
# *** NOTE ***
...
# Options:
    ...
    # Directory name used to create package related directories (like /usr/local/include/felcon)
    OPTION_PACKAGE_DIRNAME="felcon"

    # Binary to test
    OPTION_BINARY_FILE="../felcon"
...
#
    ...
    MYBUILDDIR="/home/${MYUSER}/felcon-build"
    ...
    if [ -f ../felcon -a -f ./build-felcon.sh ]; then
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    TMPDIR=$(mktemp -d /tmp/felcon-BUILDROOT.XXXXXX)
    if [ $? -eq 0 ]; then
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    if [ -f ${TARBALL} ]; then
        ...
        tar -C ${MYWORKDIR} --exclude=debian --exclude=README.md --exclude=.bzr* --exclude=.git* -c -z -f ${TARBALL} felcon 2> /dev/null
        if [ -f ${TARBALL} ]; then
    ...
    # RPM creation
    SOURCEFILE_RPM="${RPMWORKDIR}/SOURCES/felcon-${LYNIS_VERSION}.tar.gz"
    if [ -f ${SOURCEFILE_RPM} ]; then
        if [ -f felcon.spec ]; then
            # adjust version in spec file
            VERSION_IN_SPECFILE=$(awk '/^Version:/ { print $2 }' felcon.spec)
            ...
            #${RPMBUILDBINARY} --quiet -ba -bl felcon.spec 2> /dev/null
        else
            echo "[X] felcon.spec not found"
            ExitFatal
        fi

        RPMFILE="${RPMWORKDIR}/RPMS/noarch/felcon-${LYNIS_VERSION}-1.noarch.rpm"
        if [ -f ${RPMFILE} ]; then
        ...
        echo "    Hint: cp <felcon.tar.gz> ${SOURCEFILE_RPM}"
        #ExitFatal
    fi
...
#        echo "    Hint: run bzr init while being in felcon directory (or bzr init ..)"
#        ExitFatal
#    else
...
#    fi
    ...
    if [ -d ${MYBUILDDIR}/git/felcon ]; then
        echo "git clone already exists"
        rm -rf ${MYBUILDDIR}/git/felcon
    ...
    if [ -d ${MYBUILDDIR}/git/felcon/debian/ ]; then
        ...
        cp -R ../debian/* ${MYBUILDDIR}/git/felcon/debian/
        cd ${MYBUILDDIR}/git/felcon/debian/
        git add .
        git commit -m "Building process for felcon release version ${LYNIS_VERSION}"
    else
        echo "[X] Could not copy debian directory and commit changes"
    fi
    #git tag -l ${MYBUILDDIR}/git/felcon
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    echo "DONE"
    echo "    felcon (SHA1): ${SHA1HASH_LYNIS}"

    # Add hashes to script
    echo -n "- Injecting SHA1 hash into felcon script                "
    echo "-NOT DONE-"
